          ...
          {visibleStudies.map((study, index) => {
            return (
              <AnimatedSection
                key={study.slug}
                delay={index * 0.05}
                className={visibleStudies.length === 1 ? "col-span-2 h-full lg:col-span-2" : "h-full"}
              >
                <Link
                  href={`/case-studies/${study.slug}`}
                  className="group flex h-full flex-col rounded-[18px] border border-[#276864]/35 bg-[#101418] p-2.5 transition-colors hover:border-[#276864] sm:p-5 lg:p-6"
                >
                  <article className="flex h-full flex-1 flex-col">
                    <div className="flex min-w-0 flex-1 flex-col">
                      ...
                      <dl className="mb-4 mt-4 grid grid-cols-2 gap-2 sm:mb-6 sm:mt-6 sm:gap-3">
                        {study.cardMetrics.map((metric) => (
                          <div key={`${metric.value}-${metric.label}`} className="rounded-lg border border-[#1f2a2d] bg-[#0b0c10]/72 p-1.5 sm:p-3">
                            <dt className="text-[0.72rem] font-semibold leading-tight text-white sm:text-lg lg:text-xl">{metric.value}</dt>
                            <dd className="mt-1 line-clamp-2 text-[0.58rem] leading-snug text-[#dadada] sm:text-xs sm:leading-relaxed">{metric.label}</dd>
                          </div>
                        ))}
                      </dl>

                      <span className="case-study-button mt-auto inline-flex w-fit items-center whitespace-nowrap rounded-full border border-[rgba(218,218,218,0.16)] bg-[#1f2a2d] px-2.5 py-2 text-[0.72rem] font-semibold leading-none text-[#dadada] transition-colors group-hover:border-[#276864] group-hover:bg-[#276864] group-hover:text-[#d8c7a3] sm:px-3 sm:text-sm sm:leading-normal">
                        Vezi studiul de caz
